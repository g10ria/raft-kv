...
type Raft struct {
	...
	peers             []*simulatedrpc.ClientEnd // RPC end points of all peers
	majorityNeeded    int                       // shortcut to store what a majority of peers entails
	...
	isLeader          bool                      // if the server believes it is the leader
	electionTimer     time.Time                 // tracks when the current election timer is set to run out
	votesReceived     int                       // number of votes received for current election so far
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if reply.VoteGranted {
		...
		DebugPrint(dVote, "S%d (term %d) voted YES for %d (term %d)", rf.me, myOldTerm, args.CandidateId, args.Term)
	} else {
		DebugPrint(dVote, "S%d (term %d) voted NO for %d (term %d)", rf.me, myOldTerm, args.CandidateId, args.Term)
	}
}
...
//
// look at the comments in ../simulated-rpc/simulatedrpc.go for more details.
...
func (rf *Raft) sendRequestVote(server int, args *RequestVoteArgs, reply *RequestVoteReply) bool {
	...
	if ok {
		// If we are a candidate, the vote was granted, and the vote was for the term we're currently in, increment votesReceived
		if !rf.isLeader && reply.VoteGranted && args.Term == rf.currentTerm {
			rf.votesReceived += 1

			// If we have enough votes to win, become leader
			if rf.votesReceived >= rf.majorityNeeded {
				rf.isLeader = true
			...
			// No need to reset votesReceived because that gets set to 0 when a new election starts
		...
		// no reply received; the caller's periodic retry will cover this
	}
	return ok
}
...
func (rf *Raft) sendInstallSnapshot(server int, args *InstallSnapshotArgs, reply *InstallSnapshotReply) bool {
	...
	if ok {
		...
		if reply.Term > rf.currentTerm && rf.isLeader {
			...
			DebugPrint(dLeader, "S%d demotes for term %d", rf.me, rf.currentTerm)
		} else if reply.Term <= rf.currentTerm && rf.isLeader {
...
func (rf *Raft) ticker() {
	for !rf.Killed() {
		...
		if !rf.isLeader && electionTimerRanOut && rf.votedFor == -1 {
			rf.resetElectionTimer()
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if isHeartbeat {
		if len(rf.logEntries) < args.LogLength || rf.commitIndex < args.CommitIndexFr {
			// follower can't yet catch its commit index up to the leader's;
			// signal that explicitly so the leader knows to send a real append
...
func (rf *Raft) advanceCommitIndex() {
	...
	for start_n := len(rf.logEntries) - 1; start_n > rf.commitIndex; start_n-- {
		if rf.logTermsReceived[start_n] < rf.currentTerm {
			// can't commit "back into" a previous term (Raft §5.4.2), so stop looking
			break
		}
		number_matches := 1
		for i := 0; i < len(rf.peers); i++ {
		...
		if number_matches >= rf.majorityNeeded {
	...
	if ok {
		...
		if reply.Term > rf.currentTerm && rf.isLeader {
			...
			if reply.Success {
				...
				// Update nextIndex and matchIndex for this server. Use max to avoid late-arriving responses from overwriting faster ones
		...
		if rf.isLeader && !rf.Killed() {
			...
			if rf.nextIndex[server] == 1 {
				...
				// back off by one and retry (no fast-backup optimization on this path)
...
func Make(peers []*simulatedrpc.ClientEnd, me int,
	persister *tester.Persister, applyCh chan raftapi.ApplyMsg) raftapi.Raft {
	rf := &Raft{}
	rf.peers = peers
	rf.persister = persister
	rf.me = me
	rf.majorityNeeded = len(rf.peers)/2 + 1

	// Initialize raft server here
	// Start with one (empty) entry in the log
	rf.logEntries = make([]interface{}, 1)
	rf.logEntries[0] = nil // index 0 is a placeholder; real entries start at index 1
