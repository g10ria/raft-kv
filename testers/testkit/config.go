...
type Config struct {
	...
	*Groups // The server groups in the test
	...
	t   *testing.T
	net *simulatedrpc.Network // The network shared by clnts and servers
...
func MakeConfig(t *testing.T, n int, reliable bool, mks FstartServer) *Config {
	...
	cfg := &Config{}
	cfg.t = t
	cfg.net = simulatedrpc.MakeNetwork()
...
func random_handles(kvh []*simulatedrpc.ClientEnd) []*simulatedrpc.ClientEnd {
	sa := make([]*simulatedrpc.ClientEnd, len(kvh))
	copy(sa, kvh)
	for i := range sa {
		j := rand.Intn(i + 1)
		sa[i], sa[j] = sa[j], sa[i]
