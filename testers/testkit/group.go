...
type FstartServer func(ends []*simulatedrpc.ClientEnd, grp Tgid, srv int, persister *Persister) []IService
...
type Groups struct {
	mu   sync.Mutex
	net  *simulatedrpc.Network
	grps map[Tgid]*ServerGrp
}

func newGroups(net *simulatedrpc.Network) *Groups {
...
func makeSrvGrp(net *simulatedrpc.Network, gid Tgid, n int, mks FstartServer) *ServerGrp {
	sg := &ServerGrp{
		net:       net,
		srvs:      make([]*Server, n),
		gid:       gid,
...
func (sg *ServerGrp) StartServer(i int) {
	...
	labsrv := simulatedrpc.MakeServer()
	for _, svc := range srv.svcs {
		s := simulatedrpc.MakeService(svc)
		labsrv.AddService(s)
	}
