...
func makeClntTo(net *simulatedrpc.Network, srvs []string) *Clnt {
...
func makeClnts(net *simulatedrpc.Network) *Clnts {
	clnts := &Clnts{net: net, clerks: make(map[*Clnt]struct{})}
	return clnts
}

func (clnts *Clnts) makeEnd(servername string) *simulatedrpc.ClientEnd {
	name := Randstring(20)
